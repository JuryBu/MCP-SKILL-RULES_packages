...
// ── 导出（供 status 工具调用） ──
export function getLaunchTaskCount() {
    const tasks = loadRegistry();
...
export function registerLaunch(server) {
    server.tool("sandbox_launch", `长任务脱离执行。进程完全独立于 MCP，可跑数小时~数天。
...
- waitSeconds 主动等待后返回，避免频繁轮询`, LaunchParamsShape, async (params) => {
        const startTime = Date.now();
        touchActivity();
        const action = params.action;
        const appendTiming = (result) => {
        ...
        if (action === "list") {
            ...
            return appendTiming({
                content: [{ type: "text", text: `📋 任务列表 (${tasks.length}):\n${lines.join("\n")}\n` }],
            });
        }
        // ── clean ──
        if (action === "clean") {
            ...
            for (const t of toClean) {
                try {
                    fs.unlinkSync(t.stdoutLog);
                }
                catch { /* */ }
                try {
                    fs.unlinkSync(t.stderrLog);
                }
                catch { /* */ }
                cleaned++;
            }
            ...
            return appendTiming({
                content: [{ type: "text", text: `🧹 清理了 ${cleaned} 个已完成任务（剩余 ${remaining.length} 个）\n` }],
            });
        }
        // ── 需要 taskId 的操作 ──
        if (action === "status" || action === "kill") {
            ...
            if (!taskId) {
                return appendTiming({
                    content: [{ type: "text", text: "❌ 需要 taskId 参数\n" }],
                });
            }
            const tasks = loadRegistry();
            ...
            if (action === "kill") {
                if (task.status !== "running") {
                    return appendTiming({
                        content: [{ type: "text", text: `⚠️ 任务 ${taskId} 已结束 (${task.status})\n` }],
                    });
                }
                try {
                    ...
                    saveRegistry(tasks);
                    return appendTiming({
                        content: [{ type: "text", text: `🛑 已终止任务 ${taskId} (PID ${task.pid})\n` }],
                    });
                }
                catch (err) {
                    return appendTiming({
                        content: [{ type: "text", text: `❌ 终止失败: ${err}\n` }],
                    });
                }
            }
            // ── status ──
            const waitSeconds = params.waitSeconds || 0;
            const tailLines = params.tailLines || 10;
            // 主动等待
        ...
        if (!command) {
            return appendTiming({
                content: [{ type: "text", text: "❌ 启动需要 command 参数\n" }],
            });
        }
        ensureLaunchDir();
        const taskId = generateId();
        const cwd = params.cwd || process.cwd();
        const logDirParam = params.logDir;
        ...
        try {
            ...
            return appendTiming({
                content: [{ type: "text", text: `❌ 启动失败: ${err}\n` }],
            });
        }
    });
}
