...
export function registerBatch(server) {
    server.tool("sandbox_batch", `一次调用并行执行多个代码片段或命令。适用于同时安装依赖、批量测试、多文件编译等场景。

每个任务独立计时、独立超时、独立内存限制，结果互不影响。
最多 5 个任务，默认并行（maxParallel=3）。`, BatchParamsSchema.shape, async (params) => {
        ...
        const { tasks, parallel = true, maxParallel = 3, } = parsed.data;
        try {
            let results;
            ...
            for (const r of results) {
                ...
                if (r.stdout)
                    parts.push(r.stdout);
                if (r.stderr)
                    parts.push(`⚠️ ${r.stderr}`);
                if (r.truncated && r.tempFile)
                    parts.push(`📁 完整输出: ${r.tempFile}`);
                parts.push("");
            }
            const output = {
            ...
            return {
                content: [{ type: "text", text: `❌ 批量执行异常: ${err instanceof Error ? err.message : String(err)}` }],
            };
        }
    });
}
 ...
 * 并行执行（受 maxParallel 限制）
 */
async function executeParallel(tasks, maxParallel) {
