...
export function registerStatus(server) {
    server.tool("sandbox_status", `查看沙箱系统状态。包括可用环境列表、CUDA 信息、资源占用、活跃会话列表。

action:
- overview (默认): 系统资源 + 活跃会话 + 临时文件
- envs: 可用语言环境列表（Python/Node/conda/bash）
- gpu: GPU/CUDA/DirectML 详细信息
- gc: 清理临时文件 + 关闭空闲会话`, StatusParamsSchema.shape, async (params) => {
...
async function buildGpu() {
    ...
    if (!envInfo.cuda.available) {
        return {
            content: [{ type: "text", text: "🎮 GPU/CUDA: 未检测到 NVIDIA GPU" }],
        };
    }
    const lines = [
        "🎮 GPU 详细信息",
