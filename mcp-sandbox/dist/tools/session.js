...
import { createSession, execInSession, getSessionStatus, closeSession, listSessions, } from "../session-manager.js";
/**
...
export function registerSession(server) {
    server.tool("sandbox_session", `管理持久 REPL 会话。变量状态跨调用保持，适合交互式开发和调试。
...
限制：同一 MCP 进程内最多 3 个并发会话，空闲 5 分钟自动关闭。`, SessionParamsSchema.shape, async (params) => {
        ...
        const { action = "exec", sessionId, language, code, cwd, env, timeout, maxMemoryMB, maxLines } = parsed.data;
        try {
            switch (action) {
                case "start": {
                    const result = createSession(language || "python", cwd, maxMemoryMB, env);
                    if ("error" in result) {
                        return {
                ...
                case "exec": {
                    if (!sessionId) {
                        return {
                            content: [{ type: "text", text: "❌ exec 操作需要 sessionId" }],
                        };
                    }
                    if (!code) {
                        return {
                            content: [{ type: "text", text: "❌ exec 操作需要 code" }],
                        };
                    }
                    const result = await execInSession(sessionId, code, timeout);
                    // maxLines 行数截断
                ...
                case "status": {
                    if (!sessionId) {
                        return {
                            content: [{ type: "text", text: "❌ status 操作需要 sessionId" }],
                        };
                    }
                    const status = await getSessionStatus(sessionId);
                    if (!status) {
                        return {
                            content: [{ type: "text", text: `❌ 会话 ${sessionId} 不存在` }],
                        };
                    }
                    const output = {
                        content: [{
                ...
                case "close": {
                    if (!sessionId) {
                        return {
                            content: [{ type: "text", text: "❌ close 操作需要 sessionId" }],
                        };
                    }
                ...
                case "list": {
                    ...
                    if (sessions.length === 0) {
                        const output = {
                            content: [{ type: "text", text: "📋 当前没有活跃会话" }],
                        };
                        return appendTiming(output, startTime);
                    }
                    const lines = ["📋 活跃会话:"];
            ...
            return {
                content: [{ type: "text", text: `❌ 异常: ${err instanceof Error ? err.message : String(err)}` }],
            };
        }
    });
}
