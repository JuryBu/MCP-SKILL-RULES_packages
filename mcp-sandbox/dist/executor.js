 ...
 * 获取解释器命令和参数
 */
function getInterpreterArgs(language, scriptPath, code, envParam) {
    // 检查是否有 env 指定的解释器路径
 ...
 */
export function foldRepeatedBlocks(text) {
    if (!text)
        return text;
 ...
 * 按 outputMode 处理输出
 */
export function processOutput(raw, mode, maxOutput, tailLines, maxLines) {
...
export async function execute(options) {
    const { code, command, language = DEFAULTS.language, cwd, env: envParam, timeout = DEFAULTS.timeout, maxMemoryMB = DEFAULTS.maxMemoryMB, maxOutput = DEFAULTS.maxOutput, outputMode = DEFAULTS.outputMode, tailLines = DEFAULTS.tailLines, maxLines, gpu, } = options;
