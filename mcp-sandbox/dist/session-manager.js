 ...
 * 生成会话 ID（语言-序号格式）
 */
function generateSessionId(language) {
    sessionCounter++;
 ...
 * 生成注入 sentinel 的代码
 */
function wrapWithSentinel(code, sentinel, language) {
    if (language === "python") {
 ...
 * 启动 REPL 进程
 */
function spawnREPL(language, cwd, envParam) {
 ...
 * 创建新会话
 */
export function createSession(language = "python", cwd = process.cwd(), maxMemoryMB = 256, envParam) {
    // 清理僵尸会话
    cleanDeadSessions();
    // 检查会话数量限制
 ...
 */
export async function execInSession(sessionId, code, timeout = 15000) {
