...
export function registerConvertTool(server) {
    server.registerTool("web_convert", {
        title: "文件格式转换",
...
返回: 输出文件路径 + 大小 + 耗时`,
        ...
        try {
            if (!fs.existsSync(params.input)) {
                return {
                    isError: true,
                    content: [{ type: "text", text: `文件不存在: ${params.input}` }],
                };
            }
            ...
            if (params.format === "pdf") {
                ...
                if (category !== "office") {
                    return {
                        isError: true,
                        content: [{ type: "text", text: `仅 Office 文件 (DOCX/PPTX/XLSX) 支持转换为 ${params.format} 格式。当前文件类型: ${category}` }],
                    };
                }
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: `不支持的目标格式: ${params.format}` }],
                };
            }
            // 如果指定了 savePath，复制过去
            if (params.savePath) {
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `转换失败: ${message}` }],
            };
        }
    });
}
 ...
 */
async function convertWithPlaywright(filePath, _ext) {
 ...
 */
async function convertWithLibreOfficeTo(filePath, format) {
    ...
    const files = fs.readdirSync(tempDir).filter(f => f.endsWith(`.${format}`) || f.endsWith(".html") || f.endsWith(".txt"));
    if (files.length === 0) {
...
//# sourceMappingURL=convert-tool.js.map