...
import { COOKIES_BACKUP_FILE, DEFAULT_USER_AGENT, DEFAULT_TIMEOUT, QUALITY_PRESETS, appendTiming, } from "../constants.js";
...
export function registerRecordVideo(server) {
    server.registerTool("web_record_video", {
        title: "录制网页/查看视频",
 ...
 */
async function handleLocalVideo(filePath, fps, maxDuration, startTime) {
    if (!fs.existsSync(filePath)) {
        return { isError: true, content: [{ type: "text", text: `视频文件不存在: ${filePath}` }] };
    }
    ...
    if (ext !== ".mp4" && ext !== ".webm") {
        return {
            isError: true,
            content: [{ type: "text", text: `浏览器不支持 ${ext} 格式的视频。支持: .mp4, .webm\n如需查看其他格式，请先用 ffmpeg 转换。` }],
        };
    }
    ...
    try {
        ...
        for (let i = 0; i < Math.min(frameCount, maxFrames); i++) {
            ...
            // seek 到指定时间
            const frameData = await page.evaluate(async (args) => {
                const vid = document.getElementById("vid");
        ...
        return { isError: true, content: [{ type: "text", text: `视频帧提取失败: ${msg}` }] };
    }
    finally {
        if (page)
 ...
 */
async function handleWebRecording(params, startTime) {
    ...
    try {
        ...
        await page.close();
        if (!video)
            throw new Error("视频录制失败：Playwright 未生成视频");
        ...
        return { isError: true, content: [{ type: "text", text: `视频录制失败: ${message}` }] };
    }
    finally {
        if (context) {
            try {
...
//# sourceMappingURL=record-video.js.map