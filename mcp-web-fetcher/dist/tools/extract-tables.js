...
export function registerExtractTables(server) {
    server.registerTool("web_extract_tables", {
        title: "提取网页表格",
...
返回: 提取的表格数据（完整数据保存到文件，返回每表预览：表头+前3行）`,
        ...
        try {
            ...
            const tables = await page.evaluate((selector) => {
                ...
                for (let t = 0; t < tableElements.length; t++) {
                    ...
                    for (let r = startIdx; r < bodyRows.length; r++) {
                        ...
                        tds.forEach(td => cells.push(td.innerText?.trim() || ""));
            ...
            if (tables.length === 0) {
                return appendTiming({
                    content: [{ type: "text", text: "未找到任何表格。" }],
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `表格提取失败: ${message}` }],
            };
        }
        finally {
            if (page)
...
function formatMarkdown(title, meta, table) {
    let output = `${title}\n*${meta}*\n\n`;
    // 确保所有行都有统一列数
    const pad = (arr, len) => {
        while (arr.length < len)
