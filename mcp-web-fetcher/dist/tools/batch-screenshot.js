...
export function registerBatchScreenshot(server) {
    server.registerTool("web_batch_screenshot", {
        title: "批量截图",
            ...
            try {
                ...
                if (pageNumbers && pageNumbers.length > 1) {
                    ...
                    try {
                        ...
                        const pdfInfo = await page.evaluate(() => window.__mcpPdfInfo).catch(() => null);
                        ...
                        const totalPages = pdfInfo?.totalPages || await page.evaluate(() => window.__mcpPdfInfo?.totalPages || 1);
                        for (const pn of pageNumbers) {
                            if (pn > totalPages)
                                continue;
                            // 跳转到指定页
                            await page.evaluate(async (p) => {
                                const info = window.__mcpPdfInfo;
                                if (info?.goToPage)
                                    await info.goToPage(p);
                            }, pn);
                            if (isLocalFile && pdfInfo) {
                                const pdfHeight = await page.evaluate((p) => window.__mcpPdfInfo?.heights?.[p] || 1080, pn);
                                await page.setViewportSize({ width: 1920, height: Math.round(pdfHeight) });
                            }
...
//# sourceMappingURL=batch-screenshot.js.map