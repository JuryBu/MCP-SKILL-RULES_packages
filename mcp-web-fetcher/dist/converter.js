 ...
 * 检测系统上可用的转换工具
 */
export async function detectConversionTools() {
    if (detectedTools)
 ...
 * @returns 临时 PDF 文件路径
 */
export async function convertToPDF(filePath) {
    // 检查源文件存在
 ...
 */
async function convertWithLibreOffice(soffice, filePath, cacheKey) {
 ...
 */
async function convertWithLatex(compiler, filePath, cacheKey) {
    const outDir = path.join(os.tmpdir(), "mcp-web-fetcher", "converted");
    if (!fs.existsSync(outDir))
        fs.mkdirSync(outDir, { recursive: true });
 ...
 */
export async function xlsxToTextSummary(filePath) {
