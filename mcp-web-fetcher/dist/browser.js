...
import { BROWSER_USER_DATA_DIR, COOKIES_BACKUP_FILE, DEFAULT_USER_AGENT, DEFAULT_TIMEOUT, ANTI_BOT_DELAY_MIN, ANTI_BOT_DELAY_MAX, DOMAIN_REQUEST_COOLDOWN, HIGH_RISK_DOMAINS, HIGH_RISK_DELAY_MIN, HIGH_RISK_DELAY_MAX, BROWSER_ACCEPT_HEADERS, } from "./constants.js";
/**
 * 浏览器管理器 — 单例模式
...
class BrowserManager {
    ...
    launching = null;
    activePages = 0;
    static MAX_CONCURRENT_PAGES = 3;
    idleTimer = null;
    static BROWSER_IDLE_TIMEOUT = 20 * 60 * 1000; // 20 分钟无活动关闭浏览器
    // 域名级请求时间记录 — 防止同域高频请求触发封禁
    domainLastRequest = new Map();
    lastRetryCount = 0;
    /**
     * 获取或启动浏览器上下文
     */
    ...
    async navigateTo(url, options) {
        ...
        if (this.activePages >= BrowserManager.MAX_CONCURRENT_PAGES) {
            throw new Error(`已达到最大并发页面数 (${BrowserManager.MAX_CONCURRENT_PAGES})，请等待其他页面关闭后重试`);
        }
        this.activePages++;
        let page;
        try {
                ...
                if (fileCategory === 'pdf' || fileCategory === 'office' || fileCategory === 'tex') {
                    ...
                    for (let cdnAttempt = 0; cdnAttempt < 2; cdnAttempt++) {
                        try {
                            ...
                            await page.waitForFunction(() => typeof window.pdfjsLib !== 'undefined', { timeout: 10000 });
                            break;
                        }
                        catch (cdnErr) {
                            if (cdnAttempt === 0) {
                    ...
                    const renderResult = await page.evaluate(async (args) => {
                        try {
                            ...
                            window.__mcpPdfInfo = {
                                ...
                                goToPage: async (targetPage) => {
                                    ...
                                    const ctx = canvas.getContext('2d');
                                    await pdfPage.render({ canvasContext: ctx, viewport }).promise;
                                    window.__mcpPdfInfo.currentPage = targetPage;
                                    window.__mcpPdfInfo.heights[targetPage] = viewport.height;
                                },
                            };
                            return {
                                success: true,
                            ...
                            return {
                                ...
                                totalPages: 0, renderedPages: [],
                                heights: {}, firstPageHeight: 0,
                            };
                        }
     ...
     */
    async waitForContentReady(page, timeout) {
        ...
        try {
            while (Date.now() - startTime < maxWait) {
                ...
                if (currentCount === lastElementCount && currentCount > 0) {
                    ...
                    if (stableCount >= stableThreshold) {
                        console.error(`[web-fetcher] DOM 稳定: ${currentCount} 个可见元素，${Date.now() - startTime}ms`);
                        break;
                    }
                }
                else {
            ...
            if (iframeCount > 0) {
                ...
                console.error(`[web-fetcher] ${iframeCount} 个 iframe 加载检查完成`);
            }
        }
        catch {
            // 检测过程出错不影响主流程
    ...
    async scrollPage(page, count) {
        for (let i = 0; i < count; i++) {
            ...
            console.error(`[web-fetcher] 滚动 ${i + 1}/${count}，页面高度: ${prevHeight} → ${newHeight}`);
            // 如果页面高度没变，说明已经到底了
    ...
    async getCookies(domain) {
        ...
        if (domain) {
            filtered = cookies.filter((c) => c.domain === domain || c.domain === `.${domain}` || c.domain.endsWith(`.${domain}`));
        }
        // 返回去除实际值的 cookie 信息
    ...
    async saveCookies() {
        ...
        try {
            ...
            fs.writeFileSync(COOKIES_BACKUP_FILE, JSON.stringify(cookies, null, 2), "utf-8");
    ...
    cleanProfileCache() {
        ...
        if (cleanedCount > 0) {
            console.error(`[web-fetcher] Profile 缓存已清理: ${cleanedCount} 个目录`);
        }
    }
}
// 导出单例
