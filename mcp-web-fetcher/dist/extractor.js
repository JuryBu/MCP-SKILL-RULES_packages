...
import { CHARACTER_LIMIT, OUTPUT_MODE_LIMITS, OUTPUT_MODE_LINES, FOOTER_GARBAGE_PATTERNS, SPA_LAZY_LOAD_DOMAINS, SPA_SKELETON_KEYWORDS, SPA_SKELETON_THRESHOLD, } from "./constants.js";
// ===== SPA 空壳检测 =====
/**
 * 检测提取的内容是否为 SPA 空壳（页面未完成加载的空白/框架内容）
 * 返回提示语（如果检测到问题），或 null（内容正常）
 */
export function detectSPAIssue(content, url) {
    // 计算有效文本长度（去除 markdown 标记和空白）
    const plainText = content.replace(/[#*\-_\[\]()>|`\n\r]/g, '').trim();
    if (plainText.length >= SPA_SKELETON_THRESHOLD)
        return null;
    // 检查是否包含 SPA 空壳关键词
    const hasSkeletonKeyword = SPA_SKELETON_KEYWORDS.some(kw => content.includes(kw));
    // 检查是否是已知 SPA 站点
    const isSPASite = SPA_LAZY_LOAD_DOMAINS.some(domain => url.includes(domain));
 ...
 * 返回 true 表示检测到严重乱码
 */
export function detectEncodingIssue(content) {
 ...
 * 尝试使用平台专用选择器提取内容
 */
function extractByPlatformSelectors(document, _url, config, docTitle) {
    // 先移除平台特定噪音
...
export function extractContent(html, url) {
    ...
    if (platformConfig) {
        ...
        const cloneForPlatform = document.cloneNode(true);
 ...
 */
export function cleanFooterGarbage(content) {
    const lines = content.split("\n");
    // === 策略1：末尾截断 ===
    ...
    const filtered = afterCut.filter(line => {
        ...
        if (matchCount === 1 && trimmed.length <= 500)
            return false;
        return true;
    });
 ...
 */
export function compactContent(content, mode = "compact") {
    if (mode === "full")
        return content;
    const charLimit = OUTPUT_MODE_LIMITS[mode] || OUTPUT_MODE_LIMITS.compact;
    const maxBodyLines = OUTPUT_MODE_LINES[mode] ?? 3;
    // 先清理 footer 垃圾 + 标题去重
