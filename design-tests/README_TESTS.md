...
Grok / ProGrok、Antigravity CLI 与 Exa 都需要接收方自己的安装、登录态或私有配置。公开测试只验证配置入口与代码路径，不携带或调用发送方凭据。Council GC 默认只做 `dryRun`（只读预演），不会在 smoke test 中自动执行会移动或删除数据的 `apply`、`restore`、`purge`。
