...
这里由接收方自行填写。公开模板不携带发送方生日、账号链接、网页登录态或私人记忆；可保留兴趣方向、协作口味和常用平台偏好。
...
新项目 Plan/Task 放 `plans/` 文件夹（`Plan_1` / `Plan_2` / `index.md` 索引）；老项目保留旧结构，除非确认迁移。
Plan 每阶段都要细化 Markdown，Task 用可勾选待办格式。**写丰满**：执行时上下文会压缩丢失，细节不记就没了，图片归档引用。
...
- 产出文件（Word/PPT/HTML/PDF等）必须用 web-fetcher 截图做视觉检查，不能只看代码觉得对就交付
  · Office 直接 file:// 截不用转 PDF；别并发截（LibreOffice 抢目录 EPERM）；首次渲染慢约 45s，注意超时
...
- 导出对话图片：`conversation_read_original` 读自己对话→图片导出到临时路径→复制到项目 `assets/` 归档
...
日常说的「Codex」= 本地客户端（大规模协同实现/Review/材料搜索/核查），通过报对话 ID 中转协作，Windsurf 不能直接启动，只能由用户操作。
Codex CLI (`sandbox_codex`)：GPT 专属通道，`background=true` 启动 + `check(waitSeconds=45)`。GPT 需求优先走 CLI，不要 spawn GPT 子代理。
...
生命周期：
- spawn 的子代理用完主动 dispose，不留僵尸占对话位
...
网页截图/文本/交互/表格/链接提取、file://查看Office/PDF/图片/视频、格式转换(web_convert)、桌面应用调试(desktop_*)。翻墙代理访问Google/X/Reddit等。知乎/B站/微博/小红书/网易云/Reddit/X/Gmail 可由接收方自行配置登录态。
...
- 获取当前对话 ID（不要说「获取不到」）：首选 `subagent_current`（直接列出活跃 Cascade ID，多个 RUNNING 时按标题/时间确认哪个是自己）/ 用户 @mention 自带 / `conversation_read_original(action="list", dataChain="windsurf", query="关键词")`。⚠️不要 fetch 不传 `conversationId`
