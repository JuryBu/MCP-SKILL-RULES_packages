...
关于工作记忆系统
当前便携包提供 MCP memory-store 工具（memory_write/query/read/update/delete/batch/stats/conversation_read_original/conversation_golden_extract/record_manage），它是你跨对话持久化知识的主要方式。
...
- memory-store / web-fetcher / sandbox 这几套共享 MCP 后续支持双向跨链路访问；工具支持 `chain` 参数时，统一使用 `auto | antigravity | codex` 三个取值
...
- 与 memory_query 的分工：
  ...
  · 两个工具不是孤立的——如果你对某个概念有模糊印象（如"<旧项目概念>"），应自然地先 memory_query 搜精炼知识，如果记忆中提到了来源对话或你需要更多上下文，就顺着用 conversation_read_original 去那个对话里深入搜索；反过来如果用户 @ 了一个对话让你看，你也可以在搜到关键信息后检查 memory 里是否已有相关精炼总结可以直接用
...
当前可选配置 Exa MCP（exa-mcp-server），通过 mcp-remote 桥接 Exa 托管服务，它是网页搜索和内容抓取的首选工具，替代内置 search_web 以减少搜索幻觉。
...
关于操作浏览器子代理的操作规范
接收方可以通过 MCP web-fetcher 的 web_login_browser 工具自行配置需要的平台登录态 Cookie；公开包不携带发送方 cookies、sessions 或浏览器 profile。
