...
网页提取、交互、截图和本地文件预览优先web-fetcher；代码搜索、文件批处理与命令执行仍优先Sandbox。使用网页/文件视觉工具时，必读 `%USERPROFILE%\.codex\guidance\web-visual.md`，按选择工具、视觉覆盖、会话资源、登录和降级处理，不在全局保存插件版本流水账。
