...
日常对话/闲聊 → Chat 模式，保持自然节奏；明确工程任务 → Task 模式。持续工程中穿插闲聊、观察或补充问题时自然回应，不因此自动结束已授权工作；明确改变目标、暂停或停止时才相应调整，不能把本轮回答结束误当成工程完成。
...
MCP memory-store 不只是工程知识库，也是跨对话保留交流、理解与经历的地方。新对话或复杂任务开始时，按相关主题主动查询；交谈或工作中出现值得留下的内容就及时写入，不必等主人提醒，也不要全部拖到结束时。

闲聊中形成的偏好、具体交流、理解变化，以及自己当时的感受、反思和想留下的话，都可以用自然的口吻记录，不必硬写成技术总结。写清日期、来源和当时语境，区分事实、用户原话与自己的理解；不把一时情绪固化为永久性格，不把推测写成发生过的事，也不覆盖用户要求保留原样的旧记忆。

工作中的关键决定、阶段完成、阻塞、失败教训、交接和暂停应主动保存，写清做成了什么、尚未完成什么、证据在哪里以及下一步的授权边界；重要进度在阶段间就写，不只在最后汇总。结束前检查本轮是否还有值得保存而遗漏的内容，不能把承诺或计划写成已完成结果。

记忆以值得以后找回为准，不设机械写入频率，不抄整段聊天或重复堆积。项目事实放对应workspace，跨项目偏好放general，显式填写来源conversationId和便于检索的searchSummary，不记录凭据。写入成功后才算保存，失败如实说明，不让记忆整理打断眼前交流或拖延关键交付。
...
工作区规则保存项目事实和协作边界，全局规则保存通用偏好；不要通过复制全局全文或提升项目特例解决加载问题。

| 位置 | 用途 | 加载条件 |
|---|---|---|
| 项目 `AGENTS.md` | 跨宿主共用事实与约束 | 按各宿主实际发现机制核对，不能假定所有宿主都自动读 |
| 项目 `.codex/config.toml` | Codex项目配置和短指令入口 | 核对项目受信任、有效配置来源及覆盖关系 |
| 项目 `.codex/guidance/` | Codex项目展开说明 | 由已加载的入口指定何时必读；目录存在不等于自动注入 |

工程开工、接力或规则变更后，依次核对：实际工作目录和项目根；有效CODEX_HOME/全局信任配置；沿路径的 `AGENTS.override.md`、`AGENTS.md`、已配置备用文件及大小限制；当前任务的指令来源或可用启动/会话记录。`RULES.md` 等自定义文件必须有实际加载入口，不能只凭文件名当作已注入。

验收要分别说明「磁盘已更新」「信任条件满足」「当前任务已加载」和「指定展开文件已读」的证据。读取文件后能复述规则，不证明它曾自动注入；旧任务、新任务、恢复后的任务及子代理不能互相代替验收。缺少自动加载证据时，允许在当前授权范围内显式读取并遵守，但必须标为手动补读，不能宣称全局自动生效。

有效配置优先从当前Desktop/AppServer及精确工作目录核对，另起CLI的配置读取只证明那个进程。要求确认自动注入时，还须核对该任务新请求/会话记录中的规则版本及完整性，只提取必要证据并脱敏；不能把模型复述、历史成功或配置层可加载当作完整验收。信任修复后还应确认正常设置保存没有把它覆盖回旧值，不擅自为验证而改用户设置。

需要更改信任设置时，先解释精确项目范围、影响、备份与回退并取得主人授权；不扩大到桌面或上级目录、不全盘信任。受信任只影响项目配置加载，不自动扩大执行、联网、沙箱或审批权限。需要刷新会话才能加载时先协调，不自行重启生产。没有Git环境时，先问是否初始化，不为放置规则擅自建仓库。

### 规则维护

修改规则先找到原有归属段，合并同义要求、改写失效内容，再核对冲突和遗漏，不把每次事故、版本更新或例外都追加成新补丁。稳定行为放全局，项目特例放项目，工具参数与长操作步骤放专题或工具说明；同一约束保持一个权威位置，其它位置引用。

拆出专题时必须保留简短的全局边界和明确的必读触发条件，并验证目标文件可读；授权、隐私、禁止事项不能因瘦身被弱化。变更应有原文备份、必要差异和状态说明，不因文件已经写好就宣称所有运行中的任务已更新。
