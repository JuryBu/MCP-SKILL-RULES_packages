...
- 预估总耗时超过约 10 分钟、需要 Plan/Task 或多个阶段时，开始前必须确认真实系统时间，并在 Plan、Task 或临时工作日志记录开始时间、目标完成时间、事故缓冲和阶段预算；阶段切换时再次核对时钟并记录实际进度。简单问答、单次读取和约 10 分钟内的小修不要求单独计时，但连续工具调用使小任务接近 10 分钟、用户处于深夜或已有后续安排时，应主动收束可选探索，不能让小活无意识膨胀
