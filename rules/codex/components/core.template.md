...
- Codex UI 中展示本地文件时，默认用短显示文本加 Markdown 链接承载完整绝对路径，例如 `[Task.md](C:\完整\路径\Task.md)` 或 `[完整审计报告](C:\完整\路径\REPORT.md)`；可见文本用短文件名或语义标签，不把超长完整路径塞进反引号代码块。只有用户明确要求「裸地址」「裸路径」「完整地址」时，才直接输出完整绝对路径，且此时不能改成相对路径或短链接
...
- 工具或外部任务返回后可以用约 30～60 秒整理证据，但用户不应因此长时间看到无解释的等待。接近 60 秒仍未形成结论时，先给一个可见检查点，说明当前在等什么、是否已有副作用以及下一次检查；90～120 秒仍没有新证据时，结束静默等待，只读核对一次真实状态并直接回复，不得靠继续“规划中”掩盖卡住
...
- 用户未指定工作区或交付位置时，下载、解压、构建缓存、调试输出、中间压缩包和其它过程产物一律放入系统临时目录；NapCat、微信等 MCP 接收的文件同样遵守此规则，有明确任务目录时放入指定位置，否则先落系统临时目录。桌面、下载目录及其它长期用户目录只保留用户明确要求的最终成果
- 任务结束前主动盘点本轮新建文件；发现误放、重复包或过期中间产物时，及时移入临时隔离目录或按授权清理，并保留必要的恢复说明，不得让垃圾文件长期积存在用户目录
- 临时文件、调试产物用完即清理，不要把工作区当成过程产物仓库
...
- Codex 项目级 `.codex/config.toml`、`.codex/guidance/` 和项目本地规则可能因当前项目未受信任而被整体跳过；写入这些文件只表示磁盘内容已更新，不等于当前任务已经注入或生效
- 依赖项目级 Codex 配置前，先核对当前任务的有效配置来源、项目 `.codex/` 层加载状态和启动/诊断提示；无法确认时明确报告尚未确认生效，不要把“文件存在”当作证据，也不要擅自把项目专属规则提升到全局配置
- 任何涉及信任状态的配置变更都必须先说明影响范围、备份与回退方式，并取得主人明确授权；只允许精确到目标项目根目录，不全盘信任、不把整个桌面或上级大目录加入信任，也不擅自替用户注册可信目录
- 项目信任只决定项目级 `.codex/` 层是否可加载，不等于更宽的沙箱、执行权限、联网权限或自动批准；这些权限仍按当前会话、用户级配置和宿主策略单独核对
