 ...
 */
export function registerLsEntry(entry) {
 ...
 * 在 cleanup 路径调用，容忍失败
 */
export function cleanupRegistryOnExit() {
    const myPid = String(process.ppid);
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            const data = readRegistry();
            if (!data.processes[myPid])
                break; // 已不在表中
 ...
 * 从注册表移除指定 PID
 */
export function removeFromRegistry(pidStr) {
    try {
        updateRegistry(data => {
 ...
 */
export function markHeartbeatSuccess(pidStr) {
 ...
 */
export function markHeartbeatFailure(pidStr) {
 ...
 */
export function cleanDeadEntries() {
    try {
        const data = readRegistry();
        let changed = false;
...
//# sourceMappingURL=ls-registry.js.map