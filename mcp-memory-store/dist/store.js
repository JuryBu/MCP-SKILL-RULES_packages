 ...
 */
export function generateMemoryId(title, now) {
...
export function ensureDataDirs() {
    ...
    if (!fs.existsSync(generalIndexPath)) {
        const initialWsIndex = { version: 1, entries: [] };
 ...
 */
export async function withIndexLock(key, fn) {
    ...
    try {
        ...
        resolve();
        // 清理已完成的锁避免内存泄漏
        if (indexLocks.get(key) === next)
            indexLocks.delete(key);
    }
}
/** 全局索引专用锁 */
export async function withGlobalIndexLock(fn) {
 ...
 * 写入全局索引（原子写入）
 */
export function writeGlobalIndex(index) {
...
export function ensureWorkspace(workspacePath) {
    ...
    if (!fs.existsSync(indexPath)) {
        const wsIndex = { version: 1, entries: [] };
 ...
 */
export function readWorkspaceIndex(hash) {
    // general 区域有独立路径，统一走 readGeneralIndex 避免路径错误
    if (hash === "general")
        return readGeneralIndex();
    // 先查缓存
 ...
 * 读取 general 区域的索引
 */
export function readGeneralIndex() {
 ...
 * 写入工作区索引（原子写入 + 更新缓存）
 */
export function writeWorkspaceIndex(hash, index) {
 ...
 * 读取工作区元信息
 */
export function readWorkspaceMeta(hash) {
 ...
 * 写入工作区元信息
 */
export function writeWorkspaceMeta(hash, meta) {
    const metaPath = path.join(WORKSPACES_DIR, hash, "_meta.json");
    writeJsonAtomic(metaPath, meta);
}
/**
 * 通过原始路径查找工作区 hash
 */
export function findWorkspaceHash(workspacePath) {
    const hash = workspaceHash(workspacePath);
    const wsDir = path.join(WORKSPACES_DIR, hash);
 ...
 * 获取所有工作区 hash 列表
 */
export function listWorkspaceHashes() {
 ...
 * 获取记忆文件路径
 */
export function getEntryPath(hash, memoryId) {
    const dir = hash === "general" ? GENERAL_DIR : path.join(WORKSPACES_DIR, hash);
 ...
 */
export function buildMemoryFile(frontmatter, body) {
    const yamlLines = [
        "---",
        `id: ${frontmatter.id}`,
 ...
 * 读取记忆文件原始内容
 */
export function readMemoryFile(hash, memoryId) {
 ...
 * 写入记忆文件（原子写入）
 */
export function writeMemoryFile(hash, memoryId, content) {
    const filePath = getEntryPath(hash, memoryId);
    // 确保 entries 目录存在
 ...
 * 删除记忆文件
 */
export function deleteMemoryFile(hash, memoryId) {
    const filePath = getEntryPath(hash, memoryId);
    if (!fs.existsSync(filePath))
        return false;
 ...
 * 更新全局索引中某工作区的统计信息
 */
export function syncGlobalIndexForWorkspace(hash) {
    const globalIndex = readGlobalIndex();
    const wsIndex = readWorkspaceIndex(hash);
