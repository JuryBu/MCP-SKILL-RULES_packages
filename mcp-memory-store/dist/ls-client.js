...
import os from "os";
import fs from "fs";
import { readRegistry, registerLsEntry, markHeartbeatSuccess, markHeartbeatFailure, cleanDeadEntries, } from "./ls-registry.js";
// ===== 父 LS 体系 =====
/** 模块变量：进程启动时通过 ppid 初始化，生命期内不变 */
let parentLs = null;
let parentLsInitPromise = null;
let parentLsInitDone = false;
/**
 * 获取已缓存的父 LS 连接（未就绪返回 null）
 */
export function getParentLs() {
    return parentLs;
}
/**
 ...
 */
export async function initParentLs() {
    if (parentLsInitDone)
        return;
    if (parentLsInitPromise)
 ...
 */
export function discoverLsProcesses() {
    try {
        const psScript = [
 ...
 */
async function rpcCall(lsInfo, port, method, payload = {}, timeoutMs = 15000) {
 ...
 */
async function getStepCountLight(lsInfo, port, cascadeId) {
 ...
 *                   不传则退化为串行循环（冷对话路径）。
 */
async function fetchAllStepsPaged(lsInfo, port, cascadeId, totalSteps) {
    // ===== 第一页（串行，用于标定页大小）=====
    const firstResult = await rpcCall(lsInfo, port, "GetCascadeTrajectorySteps", { cascadeId, stepOffset: 0 }, HEAVY_TIMEOUT);
    ...
    const safeInterval = Math.max(50, // 最小间隔
    ...
    if (totalSteps && totalSteps > 0) {
        ...
        // 并行请求
        const pagePromises = offsets.map(offset => rpcCall(lsInfo, port, "GetCascadeTrajectorySteps", { cascadeId, stepOffset: offset }, HEAVY_TIMEOUT)
        ...
        while (allSteps.length < totalSteps) {
            const result = await rpcCall(lsInfo, port, "GetCascadeTrajectorySteps", { cascadeId, stepOffset: allSteps.length }, HEAVY_TIMEOUT);
 ...
 */
async function fetchStepsIncremental(lsInfo, port, cascadeId, fromOffset) {
    const newSteps = [];
    let offset = fromOffset;
    while (true) {
        const result = await rpcCall(lsInfo, port, "GetCascadeTrajectorySteps", { cascadeId, stepOffset: offset }, HEAVY_TIMEOUT);
        const steps = result.data?.steps ?? [];
        if (steps.length === 0)
            break; // 唯一的终止条件：返回 0 条
        newSteps.push(...steps);
        offset += steps.length;
 ...
 */
async function verifyTail(lsInfo, port, cascadeId, cachedSteps, verifyCount) {
    ...
    try {
        const result = await rpcCall(lsInfo, port, "GetCascadeTrajectorySteps", { cascadeId, stepOffset: startOffset }, LIGHT_TIMEOUT);
        const currentTailSteps = result.data?.steps ?? [];
        if (currentTailSteps.length < actualCount)
            return false; // 数据量不足，视为不一致
        // 对比每一步的 type 和关键内容
        for (let i = 0; i < actualCount; i++) {
            ...
            if (cachedText && currentText && cachedText !== currentText)
                return false;
        }
        return true;
    }
 ...
 * 3. 回溯或校验失败 → 并行分页全量重拉
 */
export async function fetchTrajectory(cascadeId, forceRefresh = false) {
    ...
    for (const [pidStr, entry] of Object.entries(registry.processes)) {
        if (pidStr === parentPidStr)
            continue; // 跳过父 LS（已在 Step 1 查过）
        // 进程存活快速检测（微秒级）
        try {
 ...
 */
async function fetchFromLs(lsInfo, port, cascadeId, cachePath, forceRefresh) {
    // 检查热列表
 ...
 */
export async function getCurrentCascadeId() {
    // 优先从父 LS 热列表获取
    if (parentLs) {
        try {
 ...
 */
export async function callGetModelResponse(model, prompt) {
    try {
        if (parentLs) {
            const result = await rpcCall(parentLs.info, parentLs.port, "GetModelResponse", {
                model, prompt,
            }, LIGHT_TIMEOUT);
        ...
        if (!port)
            return null;
        const result = await rpcCall(lsInfo, port, "GetModelResponse", { model, prompt }, LIGHT_TIMEOUT);
        return result.data?.response ?? null;
    }
    catch {
        return null;
 ...
 */
export async function generateAutoSummary(title, tags, content) {
    const prompt = `为以下记忆条目生成一段简洁的搜索优化摘要（100-200字，包含关键词和近义词）：\n\n标题: ${title}\n标签: ${tags.join(", ")}\n内容: ${content.slice(0, 3000)}`;
    return callGetModelResponse("MODEL_PLACEHOLDER_M47", prompt);
}
