...
import { readGlobalIndex, readWorkspaceIndex, readGeneralIndex, readMemoryFile, readWorkspaceMeta, findWorkspaceHash, listWorkspaceHashes, } from "../store.js";
...
export function registerQuery(server) {
    server.tool("memory_query", "查询记忆。支持三档深度控制、标签过滤、全文搜索、时间范围过滤。无参调用返回当前工作区概览。", {
    ...
    }, async ({ query, grep, scope, workspace, depth, tags, category, after, before, limit }) => {
        touchActivity();
...
// === 概览模式 ===
function handleOverview(hash, wsPath) {
    const wsIndex = hash === "general" ? readGeneralIndex() : readWorkspaceIndex(hash);
    ...
    // 构建输出
    let body = `📚 工作区: ${wsName} (${count} 条记忆)\n` +
        `状态: ${isHot ? "🔥已缓存" : "📦未缓存"}\n` +
        (topTags ? `热门标签: ${topTags}\n` : "");
    // 📌 置顶记忆区域
    if (displayPinned.length > 0) {
        const pinnedList = displayPinned.map((e, i) => `  ${i + 1}. [${e.id}] ${e.title} [${e.tags.join(",")}]`).join("\n");
        body += `\n📌 置顶记忆 (${displayPinned.length} 条):\n${pinnedList}\n`;
    }
    // 🕐 最近记忆区域
    const recentList = displayNormal.map((e, i) => `  ${i + 1}. [${e.id}] ${e.title} [${e.tags.join(",")}]`).join("\n");
...
// === grep 模式 ===
function handleGrep(pattern, scope, workspace, limit) {
    if (scope === "global") {
        const allResults = grepGlobal(pattern, limit);
        if (allResults.length === 0) {
            return {
                content: [{ type: "text", text: `🔍 全局搜索 "${pattern}" 无结果` }],
            };
        }
        let totalMatches = 0;
        const parts = [];
    ...
    if (wsPath === "general") {
        ...
        if (!found) {
            return {
                content: [{ type: "text", text: `❌ 工作区不存在: ${wsPath}` }],
            };
        }
        hash = found;
    }
    ...
    if (results.length === 0) {
        return {
            content: [{ type: "text", text: `🔍 在工作区中搜索 "${pattern}" 无结果\n💡 可用 scope="global" 搜索全局记忆` }],
        };
    }
    const parts = results.map(r => `[${r.memoryId}] ${r.title} (第 ${r.lineNumber} 行)\n  > ${r.lineContent.trim()}`);
    return {
        content: [{
                type: "text",
...
// === 格式化结果 ===
function formatResults(results, depth, hash, isGlobal = false) {
    if (results.length === 0) {
        const hint = isGlobal ? '' : '\n💡 可用 scope="global" 搜索全局记忆';
        return {
            content: [{ type: "text", text: `🔍 未找到匹配的记忆${hint}` }],
        };
    }
    if (depth === "full") {
        // full 模式：写临时文件
        ...
        for (const { entry } of results) {
            const sourceHash = entry._sourceHash || hash;
