...
import { ensureWorkspace, generateMemoryId, buildMemoryFile, writeMemoryFile, readMemoryFile, readWorkspaceIndex, writeWorkspaceIndex, syncGlobalIndexForWorkspace, findMemoryById, deleteMemoryFile, parseMemoryFile, countLines, readConfig, findWorkspaceHash, readGeneralIndex, } from "../store.js";
...
export function registerBatch(server) {
    server.tool("memory_batch", "一次调用执行多个记忆操作。适用于批量导入、链式查询、多条更新。最多 20 个操作。", {
            ...
            try {
                ...
                switch (op.action) {
                    ...
                    case "read": {
                        ...
                        if (!found) {
                            result = `❌ read: 记忆不存在 ${op.id}`;
                            failCount++;
                            break;
                        }
                        const content = readMemoryFile(found.hash, op.id);
                        if (!content) {
                            result = `❌ read: 文件丢失 ${op.id}`;
                            failCount++;
                            break;
                        }
                    ...
                    case "query": {
                        ...
                        if (op.grep) {
                            ...
                            if (grepResults.length === 0) {
                                ...
                                const parts = grepResults.map(r => `  [${r.memoryId}] ${r.title} (L${r.lineNumber}): ${r.lineContent.trim()}`);
                        ...
                        if (op.query) {
                            ...
                            if (searchResults.length === 0) {
                                ...
                                const parts = searchResults.map(r => `  [${r.entry.id}] ${r.entry.title} (${(1 - r.score).toFixed(2)})`);
                    ...
                    case "update": {
                        ...
                        const found = findMemoryById(op.id);
                        if (!found) {
                            result = `❌ update: 不存在 ${op.id}`;
                            failCount++;
                            break;
                        }
                        const content = readMemoryFile(found.hash, op.id);
                        if (!content) {
                            result = `❌ update: 文件丢失 ${op.id}`;
                            failCount++;
                            break;
                        }
                        const parsed = parseMemoryFile(content);
                        if (!parsed) {
                            result = `❌ update: 格式异常 ${op.id}`;
                            failCount++;
                            break;
                        }
                        ...
                        if (op.title)
                            fm.title = op.title;
                        if (op.searchSummary)
                            fm.searchSummary = op.searchSummary;
                        if (op.tags) {
                        ...
                        const newFm = {
                            ...
                            tags: (Array.isArray(fm.tags) ? fm.tags : found.entry.tags),
                        ...
                        if (ie) {
                            if (op.title)
                                ie.title = op.title;
                            if (op.searchSummary)
                                ie.searchSummary = op.searchSummary;
                            if (op.tags)
                                ie.tags = [...new Set([...ie.tags, ...op.tags])];
                            if (op.pinned !== undefined)
                                ie.pinned = op.pinned;
