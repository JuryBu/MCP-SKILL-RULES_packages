...
import { parseRounds, formatRound, formatOverview, saveConversationToTemp, searchInRounds, } from "../trajectory.js";
/**
 ...
 *   read   — 读取指定轮次范围的对话内容
 */
export function registerConversation(server) {
    server.tool("conversation_read_original", `读取对话的原始完整内容（绕过上下文压缩机制）。
三种操作模式:
...
不填 conversationId 默认读取当前对话。`, {
        ...
        try {
            const { action, conversationId, query, depth = "normal", contextRounds = 2, limit = 8, startRound, endRound, extraTypes = [], } = params;
            // 确定对话 ID
            ...
            if (action === "search") {
                if (!query) {
                    return appendTiming({
                        content: [{ type: "text", text: "❌ search 模式需要提供 query 参数" }],
                    }, startTime);
                }
                const matches = searchInRounds(rounds, query, limit);
                if (matches.length === 0) {
                    return appendTiming({
                        content: [{ type: "text", text: `🔍 搜索 "${query}" — 未找到匹配` }],
                    }, startTime);
                }
                const output = [];
                output.push(`🔍 搜索 "${query}" — 命中 ${matches.length} 处\n`);
                // 收集需要展示的轮次（去重 + 上下文）
                const roundsToShow = new Set();
                ...
                for (const ri of sortedRounds) {
                    ...
                    if (!round)
                        continue;
                    output.push(formatRound(round, depth, extraTypes));
                    output.push("");
                }
                // 上下文大小控制
            ...
            if (action === "read") {
                ...
                if (start > rounds.length) {
                    return appendTiming({
                        content: [{ type: "text", text: `❌ startRound ${start} 超出范围（共 ${rounds.length} 轮）` }],
                    }, startTime);
                }
                const output = [];
                const overview = formatOverview(cascadeId, rounds, steps.length);
                ...
                for (let i = start; i <= Math.min(end, rounds.length); i++) {
                    output.push(formatRound(rounds[i - 1], depth, extraTypes));
                    output.push("");
                }
                // 上下文大小控制
                let text = output.join("\n");
