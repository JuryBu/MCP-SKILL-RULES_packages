...
import { callGetModelResponse, isLsAvailable, fetchTrajectory, getCurrentCascadeId } from "../ls-client.js";
import { parseRounds } from "../trajectory.js";
 ...
 * v1.5 新增工具
 */
export function registerGoldenExtract(server) {
    server.tool("conversation_golden_extract", "从对话中提取关键信息（决策、发现、踩坑经验），并与现有记忆对比去重。帮助发现对话中值得持久化但尚未保存的知识。", {
    ...
    }, async ({ conversationId, stepStart, stepEnd, autoCompare, workspace }) => {
        touchActivity();
        ...
        try {
            ...
            if (!lsOk) {
                return appendTiming({
                    content: [{ type: "text", text: "❌ LS 不可用，无法提取黄金片段。请确保 IDE 正在运行。" }],
                }, startTime);
            }
            // 确定对话 ID（与 conversation_read_original 相同的逻辑）
            const cascadeId = conversationId || await getCurrentCascadeId();
            if (!cascadeId) {
                return appendTiming({
                    content: [{ type: "text", text: "❌ 未指定 conversationId 且无法推断当前对话" }],
                }, startTime);
            }
            // 获取对话 trajectory
            let result;
            try {
                result = await fetchTrajectory(cascadeId);
            }
            catch (err) {
                return appendTiming({
                    content: [{ type: "text", text: `❌ 获取对话数据失败: ${err.message}` }],
                }, startTime);
            }
            if (!result) {
                return appendTiming({
                    content: [{ type: "text", text: `❌ 对话数据为空: ${cascadeId}` }],
                }, startTime);
            }
            ...
            if (stepStart !== undefined || stepEnd !== undefined) {
                ...
                const end = stepEnd || steps.length;
                steps = steps.slice(start, end);
            }
            // 使用 parseRounds 正确解析对话轮次（复用 trajectory.ts 的逻辑）
            const rounds = parseRounds(steps);
            if (rounds.length === 0) {
                return appendTiming({
                    content: [{ type: "text", text: "⚠️ 对话步骤中未解析到有效轮次。" }],
                }, startTime);
            }
            // 从轮次中提取对话文本
            ...
            if (dialogueText.length < 50) {
                return appendTiming({
                    content: [{ type: "text", text: "⚠️ 对话内容过短，无法提取有价值的片段。" }],
                }, startTime);
            }
...
请直接输出提取结果，每行一条，不要加序号或其他格式。如果没有值得提取的信息，输出"无"。`;
            const flashResponse = await callGetModelResponse("MODEL_PLACEHOLDER_M47", prompt);
            if (!flashResponse || flashResponse.trim() === "无") {
                return appendTiming({
                    content: [{ type: "text", text: "ℹ️ 对话中未发现需要提取的关键信息。" }],
                }, startTime);
            }
            // 解析 Flash 输出
            const fragments = parseFragments(flashResponse);
            ...
            let resultText = `🔍 黄金片段提取 — ${cascadeId.slice(0, 8)}... (${steps.length}步, ${rounds.length}轮)\n\n`;
 ...
 */
function extractDialogueFromRounds(rounds) {
    const parts = [];
 ...
 * 收集所有可搜索的记忆条目
 */
function collectAllEntries(targetWorkspace) {
    const allEntries = [];
    if (targetWorkspace) {
...
//# sourceMappingURL=golden-extract.js.map