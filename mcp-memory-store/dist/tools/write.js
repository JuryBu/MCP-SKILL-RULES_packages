...
import { ensureWorkspace, generateMemoryId, buildMemoryFile, writeMemoryFile, readMemoryFile, readWorkspaceIndex, writeWorkspaceIndex, syncGlobalIndexForWorkspace, countLines, readConfig, parseMemoryFile, } from "../store.js";
 ...
 */
export function registerWrite(server) {
    server.tool("memory_write", "写入一条新的记忆。自动检测当前工作区下是否有相似记忆并提醒。searchSummary 可选（v1.5+），系统会自动用 Flash 生成 autoSummary 补充。", {
    ...
    }, async ({ title, content, searchSummary, tags, workspace, category, conversationId, pinned }) => {
        touchActivity();
        ...
        try {
            ...
            if (duplicates.length > 0) {
                dedupWarning = "\n\n⚠️ 发现相似记忆：\n" +
                    duplicates.slice(0, 3).map(d => `- [${d.entry.id}] "${d.entry.title}" (相似度: ${d.score.toFixed(2)})\n  如需合并请用 memory_update(id="${d.entry.id}", append="...")`).join("\n");
            }
            ...
            triggerAutoSummary(hash, id, title, tags, content).catch(() => { });
            return appendTiming({
                content: [{
                        type: "text",
 ...
 * 成功后更新记忆文件和索引
 */
async function triggerAutoSummary(hash, memoryId, title, tags, content) {
    try {
        const summary = await generateAutoSummary(title, tags, content);
        if (!summary)
            return; // LS 不可用，静默跳过
