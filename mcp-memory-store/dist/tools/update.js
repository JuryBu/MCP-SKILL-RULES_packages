...
import { findMemoryById, readMemoryFile, writeMemoryFile, readWorkspaceIndex, writeWorkspaceIndex, syncGlobalIndexForWorkspace, parseMemoryFile, buildMemoryFile, countLines, readConfig, } from "../store.js";
...
export function registerUpdate(server) {
    server.tool("memory_update", "更新已有记忆的内容、标题、标签、分类或搜索摘要。支持追加模式（append）。", {
        ...
        try {
            ...
            const newFrontmatter = {
                ...
                tags: (Array.isArray(fm.tags) ? fm.tags : entry.tags),
            ...
            if (indexEntry) {
                ...
                if (searchSummary !== undefined)
                    indexEntry.searchSummary = searchSummary;
                if (tags !== undefined)
                    indexEntry.tags = [...new Set([...indexEntry.tags, ...tags])];
                if (removeTags !== undefined)
                    indexEntry.tags = indexEntry.tags.filter(t => !removeTags.includes(t));
                if (category !== undefined)
                    indexEntry.category = category;
                if (pinned !== undefined)
                    indexEntry.pinned = pinned;
            ...
            if (contentChanged) {
                ...
                triggerAutoSummaryUpdate(hash, id, newFrontmatter.title, (Array.isArray(newFrontmatter.tags) ? newFrontmatter.tags : []), body).catch(() => { });
            }
            return appendTiming({
                content: [{
                        type: "text",
 ...
 */
async function triggerAutoSummaryUpdate(hash, memoryId, title, tags, body) {
    try {
