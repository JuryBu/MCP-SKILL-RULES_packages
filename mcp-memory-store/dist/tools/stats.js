...
import { WORKSPACES_DIR, GENERAL_DIR, readGlobalIndex, readWorkspaceIndex, writeWorkspaceIndex, readWorkspaceMeta, writeWorkspaceMeta, listWorkspaceHashes, syncGlobalIndexForWorkspace, workspaceHash, parseMemoryFile, ensureDataDirs, writeJsonAtomic, buildMemoryFile, } from "../store.js";
import { indexCache } from "../cache.js";
import { cleanOldTempFiles, TEMP_DIR } from "../temp-store.js";
import { generateAutoSummary, isLsAvailable } from "../ls-client.js";
/**
 ...
 * - resolve 后必须仍在目标目录内
 */
function isSafeFilename(entriesDir, filename) {
    // 层1: 黑名单字符检查
 ...
 */
export function registerStats(server) {
    server.tool("memory_stats", "查看记忆系统统计信息，管理冷热归档，导出/导入记忆。", {
    ...
    }, async ({ action, workspace, ids, savePath, zipPath }) => {
        touchActivity();
        ...
        try {
            switch (action || "overview") {
                ...
                case "enhance":
                    return appendTiming(await handleEnhance(workspace), startTime);
                default:
                    return appendTiming({ content: [{ type: "text", text: `❌ 未知 action` }] }, startTime);
            }
        }
        catch (error) {
            return appendTiming({
...
function handleOverview() {
    ...
    if (fs.existsSync(TEMP_DIR)) {
        ...
        for (const f of tempFiles) {
            try {
                tempSize += fs.statSync(path.join(TEMP_DIR, f)).size;
            }
            catch { }
        }
    }
...
// === detail ===
function handleDetail(workspace) {
    if (!workspace) {
        return { content: [{ type: "text", text: "❌ detail 需要指定 workspace" }] };
    }
    const hash = workspaceHash(workspace);
    const meta = readWorkspaceMeta(hash);
    if (!meta) {
        return { content: [{ type: "text", text: `❌ 工作区不存在: ${workspace}` }] };
    }
...
function handleGc() {
    ...
    for (const hash of hashes) {
        const meta = readWorkspaceMeta(hash);
        if (!meta || !meta.originalPath)
            continue;
        // 跳过 "general"（无原始路径）
        if (meta.originalPath === "general")
            continue;
...
function handleArchive(workspace) {
    if (!workspace)
        return { content: [{ type: "text", text: "❌ archive 需要指定 workspace" }] };
    const hash = workspaceHash(workspace);
...
// === unarchive ===
function handleUnarchive(workspace) {
    if (!workspace)
        return { content: [{ type: "text", text: "❌ unarchive 需要指定 workspace" }] };
    const hash = workspaceHash(workspace);
    const wsDir = path.join(WORKSPACES_DIR, hash);
    const archivePath = path.join(wsDir, "_archive.gz");
    const entriesDir = path.join(wsDir, "entries");
    if (!fs.existsSync(archivePath)) {
        return { content: [{ type: "text", text: "❌ 未找到归档文件" }] };
    }
...
// === export ===
function handleExport(workspace, ids, savePath) {
    if (!savePath)
        return { content: [{ type: "text", text: "❌ export 需要指定 savePath" }] };
    ...
    for (const hash of hashes) {
        ...
        if (fs.existsSync(entriesDir)) {
            ...
            for (const file of files) {
                ...
                // 如果指定了 ids，只导出指定的
                if (ids && ids.length > 0 && !ids.includes(memId))
                    continue;
        ...
        if (Object.keys(wsData.entries).length === 0 && fs.existsSync(archivePath)) {
            try {
                const compressed = fs.readFileSync(archivePath);
                const archive = JSON.parse(gunzipSync(compressed).toString("utf-8"));
                for (const [filename, content] of Object.entries(archive)) {
...
// === import ===
function handleImport(zipPath) {
    if (!zipPath)
        return { content: [{ type: "text", text: "❌ import 需要指定 zipPath" }] };
    if (!fs.existsSync(zipPath))
        return { content: [{ type: "text", text: `❌ 文件不存在: ${zipPath}` }] };
    // 确保数据目录存在
    ensureDataDirs();
    ...
    if (importData.workspaces) {
        for (const [hash, wsData] of Object.entries(importData.workspaces)) {
            const wsDir = path.join(WORKSPACES_DIR, hash);
            const entriesDir = path.join(wsDir, "entries");
            ...
            // 合并记忆
            let unsafeFiles = 0;
            for (const [filename, content] of Object.entries(wsData.entries)) {
                if (!isSafeFilename(entriesDir, filename)) {
                    unsafeFiles++;
                    continue;
                }
                ...
                if (fs.existsSync(targetPath)) {
                    ...
                    const incoming = parseMemoryFile(content);
    ...
    if (importData.general?.entries) {
        ...
        fs.mkdirSync(entriesDir, { recursive: true });
        for (const [filename, content] of Object.entries(importData.general.entries)) {
            if (!isSafeFilename(entriesDir, filename)) {
                skipped++;
                continue;
            }
...
// === enhance ===
async function handleEnhance(workspace) {
    // 检查 LS 可用性
    const lsOk = await isLsAvailable();
    if (!lsOk) {
        return { content: [{ type: "text", text: "❌ LS 不可用，无法生成 autoSummary。请确保 IDE 正在运行。" }] };
    }
    // 收集需要增强的记忆
    const targets = [];
    let skipped = 0;
    ...
    if (targets.length === 0) {
        return { content: [{ type: "text", text: `✅ 无需增强，所有 ${skipped} 条记忆已有 autoSummary` }] };
    }
    let success = 0;
    let failed = 0;
    for (const target of targets) {
        try {
            ...
            const summary = await generateAutoSummary(target.entry.title, target.entry.tags, parsed.body);
            if (!summary) {
                failed++;
                continue;
            }
 ...
 * 从 entries 目录重建工作区索引
 */
function rebuildWorkspaceIndex(hash) {
