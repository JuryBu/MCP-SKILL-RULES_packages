 ...
 */
function substringMatchScore(entry, token) {
 ...
 */
export function fuseSearch(entries, query, limit = 10) {
    if (entries.length === 0)
        return [];
    // === 单词查询：直接走 fuse.js ===
 ...
 */
function singleTokenSearch(entries, query, limit) {
 ...
 */
function multiTokenSearch(entries, query, limit) {
 ...
 * 去重相似度检测（也使用混合策略）
 */
export function checkDuplicates(entries, title, searchSummary, threshold = 0.2) {
    if (entries.length === 0)
        return [];
    const resultMap = new Map();
 ...
 */
export function grepInEntries(hash, pattern, limit = 20) {
 ...
 */
export function grepGlobal(pattern, limit = 20) {
    const allResults = [];
    let totalCount = 0;
    // 先搜 general
