...
`codexWakeMessageVisibility` 只控制自动唤醒的界面显示，不删除服务端历史，也不改变模型收到的内容或业务确认。`hidden` 的设计语义是空闲时开启一轮的首条提醒可见，运行中追加的提醒隐藏；`visible` 则全部可见。可确定活动轮次时使用 `turn/steer`，否则按现有注入规则处理；当前官方 `turn/start` 本身也可向活动轮次追加输入，不能只凭方法名判断是否新开一轮。省略或设为 `visible` 时会持久化 UUID 格式的 `clientUserMessageId`，`hidden` 不携带该 ID，但省略 ID 不等于所有 Desktop 版本都能实时正确隐藏。

针对 Desktop `26.915.4065.0` 的实时视图与历史重建差异，外层代理提供窄范围显示兼容：只处理本桥预登记并绑定到明确服务端消息的隐藏提醒，保留开头、人工消息、visible 模式和原始历史。未知轮次、无法确认的归属及其它协议消息原样通过。该兼容层的边界、准备和验收见 [隐藏提醒兼容说明](docs/wake-visibility.md)。task router 每次唤醒前重新读取 binding，修改显示偏好无需重启路由器；升级外层代理代码则必须核对实际进程是否加载新版本，不能把复制文件当作运行生效。
...
普通模型请求默认在首次或后续连续 40 秒没有实际正文、推理增量或工具参数时终止当前上游连接，状态心跳不刷新计时。可恢复中断通过协议交给 Codex 原生重试，不在代理内部额外叠加一套普通请求重放；每条请求独立计时，不重启代理或中断其它对话。默认最多初次请求加 5 次重试，第五次快速失败仅在整条重试链累计不足 40 秒时补足等待，不增加请求次数。

普通流式模式到达 40 秒有效进度期限时，若仍有真实上游保活、尚未确认持续流式且不处于工具准备/完成阶段，可进入一次集中交付探测。探测总期限从请求开始计算，最多 300 秒；同一来源、账号和模型最多每 10 分钟获得一次探测机会，并发请求不能重复领取。成功完成的集中交付才记为集中模式，后续请求直接使用 300 秒总期限；迟到但真实分段输出的成功响应不改变下一请求的 40 秒默认检查。短流证据不足时保持未知，不反推为集中模式。

集中判定要求正文或工具参数在不超过 100 毫秒的短时段集中到达且累计至少 256 字符；流式恢复要求同一输出通道至少 8 个增量、80% 为不超过 256 字符的小段、持续至少 1.5 秒并跨越至少 4 个时间段。使用网络数据块接收时间，不把同一批数据解析出的许多事件误认成持续流式。只有完整成功才更新状态，失败、取消和压缩控制不更新交付模式。

探测和集中模式均有独立的 90 秒上游失活期限，没有首帧时从请求开始计算；真实事件或已验证的部分内容可以刷新连接活跃时间，但绝不延长 300 秒总期限。总期限或失活期限到达后停止本次生成，不再交给原生重试重复等候，同一轮的重复请求也不会重新生成；提交工具完成事件前再次核对绝对期限。用户仍可随时取消。现有工具准备/完成期限可能更早到达，普通持续流式请求并没有新增 300 秒总时长限制。

模式只按来源、账号和模型的组合哈希隔离保存到私有 `state/codex-model-adaptive-delivery.json`，最多 256 项，24 小时过期；不保存原始账号、请求正文或认证信息，损坏状态会记录诊断并退回默认模式。日志记录探测、模式改变、迟到流式和预算停止，不弹窗、不发 QQ 提醒、不伪造模型进度。runner 的 `CODEX_MODEL_STREAM_PROXY_ADAPTIVE_WAIT_LIMIT_MS` 默认及最大值为 `300000`，`CODEX_MODEL_STREAM_PROXY_UPSTREAM_IDLE_TIMEOUT_MS` 默认为 `90000`。

客户端函数调用已开始但尚未交付参数时，每条请求可使用一次固定 120 秒准备期限；重复开始事件和心跳不能延长，真实参数增量或参数完成会退出准备期限。准备或工具完成阶段的本地超时明确区分于网络断流；仅限未交付参数的纯本地函数准备，或尚未放行的上下文控制完成确认，在没有已交付可执行工具或实质内容时允许最多3次请求的有限原生重试，其余本地阶段超时停止并说明原因。带非空加密内容且身份未重复的推理完成项可以刷新空闲计时，但不代表工具执行或业务完成，也不能续长固定准备期限。

已有文字或工具名称不再被误判为工具已经执行，因此不会仅凭这些事件禁止重试。客户端可执行的工具完成事件会暂存，只有收到真正的 `response.completed` 后才交付给 Codex；断流后的保护性收尾不会补成一个可执行工具调用。恢复模块保留上游响应身份和已经输出的信息。重试耗尽后保留信息并正常结束当前回合，明确的额度或永久错误不无尽重试；同一任务连续三个不同回合都没有业务内容时保留真实失败，供原生 Goal 停止自动续跑。

`new_context` 属于受保护的上下文控制工具，不因工具名称出现就放弃重试。旧摘要压缩路径仍要求真实成功结果，失败不得伪造摘要；它的失败不适用普通回合的正常收尾保证。预热及独立记忆接口保持各自路径，不把普通模型流保护扩大成对所有接口的承诺。代理不监控本地 MCP、Sandbox 或工具执行时间，本地工具完成后的下一次模型请求会得到一个新的独立看门周期。公开更新不会替接收机启用实验性功能或覆盖其私有配置。

热更新兼容性检查将 `adaptive-delivery.mjs`、`codex-stream-recovery.mjs`、`tool-preparation-deadline.mjs`、`partial-response-progress.mjs`、`tool-delivery-profile.mjs` 和 `reasoning-progress.mjs` 与模型流代理主模块一起视为受保护文件；只改这些依赖也不能走仅后端热更新。代理会把未完成 SSE 帧中的语义内容增长视作进度，忽略重复心跳和重复 reasoning；私有哈希 profile 精确命中时，工具准备窗由默认 120 秒延长至 300 秒，首个参数增量立即切回 40 秒正常 idle。
...
有人值守恢复时直接运行 `ops/start-napcat-login.ps1`：脚本不传 `-NoQr` 时走普通人工登录流程，不读取 DPAPI 密码回退凭据，也不强行把 binding 中的 `expectedSelfId` 拼成快速登录参数，避免快速票据失效时卡在隐藏进程里却不显示二维码或安全验证。脚本同时兼容 Node 一键包的 `napcat/cache/qrcode.png` 与旧 Shell 布局的 `cache/qrcode.png`；人工模式遇到 QQ 安全验证链接时会把链接复制到剪贴板并延长等待窗口，让浏览器验证有时间回写登录票据。监督器默认传 `-NoQr -NoPasswordFallback`，只尝试该账号已有的本地快速登录票据，不在无人值守路径撞密码或验证码。检测到二维码、安全验证、人机验证、登录态失效，或 DPAPI 凭据无法解密时，登录脚本返回 `NAPCAT_MANUAL_LOGIN_REQUIRED` 并持续阻断自动登录，不再按 30 分钟复查窗口重新拉起登录器。真实 OneBot 在线会立即解除阻断；需要重新尝试人工登录时必须由有人值守的显式登录脚本或外部恢复动作触发，因此不会在无人值守桌面弹二维码，也不会按普通冷却周期反复制造登录尝试。
