...
function Invoke-NpmChecked {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "npm $($Arguments -join ' ') failed while restoring the previous release." }
  } finally {
...
try {
  ...
  if ($PSCmdlet.ShouldProcess($CodeRoot, "restore the previous validated NapCat bridge code")) {
    ...
    try {
      Invoke-NpmChecked -Arguments @("ci")
      Invoke-NpmChecked -Arguments @("run", "check")
