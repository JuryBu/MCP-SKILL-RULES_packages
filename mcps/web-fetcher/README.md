...
- `web_human_browser_status` 返回页面 URL/title、存活状态、Cookie 数量和最近 challenge 检测结果。
...
- `web_human_browser_detach` 只断开 MCP/CDP 引用，不关闭真实 Chrome；`web_human_browser_close` 会关闭受管 Chrome 并清理临时 profile，attach 外部浏览器时只释放引用。
...
后台登录任务会在 Chrome 窗口关闭后导出 Cookie；如果还没关闭，查询会返回运行中状态。

## MCP 配置

配置文件位置按接收方 MCP 宿主决定；Codex 用户优先使用本工具包 `templates/config.codex.toml` 与 `install/Apply-CodexConfig.ps1`。
...
```json
{
  "mcpServers": {
    "web-fetcher": {
      "command": "node",
      "args": ["%USERPROFILE%\\.codex-toolkit\\mcps\\web-fetcher\\dist\\index.js"],
      "env": {}
    }
  }
}
