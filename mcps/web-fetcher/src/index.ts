...
server.registerResource(
    ...
    async () => {
        ...
        const guide = [
            ...
            "- 请求按 ownerId 公平排队，web_interact/web_pipeline 复用 sessionId 时按实际 Page 串行；直接 desktop_* 与同窗口网页 alias 仍需调用方串行。关闭/取消/状态查询使用独立控制通道。返回的 _meta.webFetcherTiming 与 _meta.webFetcherPool 提供耗时和池状态，不能仅用总耗时判断网页加载速度。",
            "- 普通 click 已确认不会下载/开新窗口时，可显式 waitForEvents=false（pipeline 在 click 步骤设置），跳过约 8 秒事件探测；省略仍保留旧下载/弹窗收集。快速点击不等待异步业务更新，需要时追加 wait；截图仍等待视觉资源。web_inspect 仍需 url，不支持用 sessionId 替代。",
