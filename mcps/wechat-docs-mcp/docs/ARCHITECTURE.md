...
`SafeTextOutbound` 是安全状态机，Windows 后端负责受控 UI 动作。执行顺序为：验证私有 route → 消费批准并获取单发送租约 → 在 wake 前保存稳定环境快照 → 可见导航并核对空草稿 → 用 `SessionDraft` 证明精确 username → 可选隐藏已验证窗口 → 每个写入、清理或发送动作前复核窗口、前台、鼠标、Windows 最近输入时间和剪贴板序列 → 尝试发送 → 最外层恢复环境。快照过程中或隐藏阶段检测到用户输入就终止；完全没有可见导航的隐藏发送不在正式能力内。

文字发送键动作完成只表示 `SEND_ATTEMPTED`。异常发生在可能发送之后，或发送后环境恢复失败，状态必须是 `UNKNOWN`，并禁止自动重试。只有可信数据库解析层中 `status=2 AND origin_source=1` 的唯一新行同时匹配精确 route 与完整正文，才可进入 `VERIFIED`。附件的本机数据库匹配只能留下发送尝试证据；还须取得带用户来源引用的接收端可见且可下载或已下载确认，才可提升为 `VERIFIED`。

当前源码已有 Windows 文字与附件 UI 后端、`SessionDraft` 草稿归属检查和数据库 outbound 方向确认，但生产总开关仍默认关闭。附件以严格无窗口 `CF_HDROP` 作为产品首选候选，但在精确隐藏 route 身份、全程可见窗口数为零、复杂剪贴板恢复和接收端可下载四项证据同时成立前保持 `candidate=true/enabled=false`；标准文件选择器是当前低打扰回退，不得描述为无窗口。每次执行的审计结果记录可见时长、前后可见窗口数、前台焦点、鼠标、剪贴板语义和因用户交互跳过恢复的状态；前后观测不能冒充全程零窗口证明。能力查询必须分别显示完全隐藏发送、附件输入方式、复杂剪贴板恢复和接收端确认要求，不能把离线或本机证据冒充真实跨端发送成功。
