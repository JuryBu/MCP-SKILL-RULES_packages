...
- Council 运行时目录优先使用 `SANDBOX_COUNCIL_ANTIGRAVITY_CLI_TEMP_DIR`，兼容旧的 `SANDBOX_COUNCIL_GEMINI_CLI_TEMP_DIR`；未设置时优先写入 `${SANDBOX_DATA_ROOT}/temp`，最后才回退到包目录的 `sandbox-data/temp`。部署 broker 时应设置 `SANDBOX_DATA_ROOT`，避免 task、索引和制品回写源码树
