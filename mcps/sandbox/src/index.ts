...
- v1.17.3 用事故附近压力值回放验证小请求通道：新鲜完整Windows采样、无低内存信号且完整提交预算守住4096MB时，未观测预留按25%计入物理风险，新请求与提交预算不打折；未知采样和后台资源恢复期间暂停新执行，观测抵扣逐租约封顶。launch改为增量异步维护、终态不重写、单任务直读与有界日志尾读；admissionDecision返回具体阻塞条件。
