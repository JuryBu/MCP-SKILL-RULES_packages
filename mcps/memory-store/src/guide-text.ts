...
- Conversation 主子线程与消息角色过滤(v1.15.11): conversation_read_original(list/export) 支持 threadMode="main|children|all"、parentConversationId、parentQuery；默认 main 会把命中子线程的标题回指父线程候选。read 支持 messageRoles=["user","system","model","assistant","tool","subagent"]；user 只含真实用户与结构化批注，subagent 单独返回带昵称和对话 ID 的子代理事件。Codex 标题定位使用全量 session_index 轻量索引，不再被最近 300 条正文候选限制。
...
- 四宿主 fetch 缓存与语义(v1.22+): fetch 建立可复用的规范化缓存，search/read/full/diff 不重复解析原始 JSONL/PB；source=auto|local|ls|cache 控制来源，约 100K 结果通过 continuationCursor 继续读取；annotations 按条展示并按命中字段搜索，子代理不增加人类轮。
- 离线目录与压缩回溯(v1.22.3): conversation_read_original(list, source="local", dataChain="windsurf|antigravity") 从 active/implicit PB 元数据列出本地对话，不依赖 IDE/LS。action="recall" 支持 recallMode="auto|manual|full"：先更新并提交 fetch cache generation，再只从同一 generation 恢复人类可见上下文；auto 补到压缩前规模约 60%，manual 使用 startRound/endRound，full 写临时 artifact。Recall 排除 thinking、工具结果、diff、Rules 注入、压缩摘要与诊断；普通大结果沿用约 100K continuation，Antigravity 无可靠压缩标记时回溯最近约 150K tokens（约 450K context characters）。
