                    ...
                    switch (op.action) {
                        ...
                        case "query": {
                            ...
                            if (op.query) {
                                ...
                                const blocks = wsIdx.entries.map(e => ({
                                    id: e.id,
                                    title: e.title,
                                ...
                                if (requestedMode === "auto" && engineResults.length === 0) {
                                    engineResults = await engineSearch(blocks, op.query, {
                                        mode: "smart",
                                        limit: op.limit || 5,
                                        modelChain: chains.modelChain,
                                    });
                                ...
                                if (engineResults.length === 0) {
                                    ...
                                    const parts = engineResults.map(r =>
                                        `  [${r.id}] ${r.title} (${r.matchType}, score: ${r.score.toFixed(2)})`
                                    );
                                    result = `🔍 "${op.query}" 找到 ${engineResults.length} 条:\n${parts.join("\n")}`;
                                }
                                successCount++;
                                break;
                            }
