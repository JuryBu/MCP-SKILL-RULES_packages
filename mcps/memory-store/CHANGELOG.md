...
- 修复 Codex 继承历史中显式序号跳号被误判为物理记录缺失，同步、异步与增量读取统一校验，保留无显式序号的旧格式和物理步数语义。
- 对选定前缀计算完整内容摘要，并把首行继承关系、边界及实际解析字节绑定；等长改写、首行/正文混合捕获、旧 manifest 升级、坏源及取消不会发布半代缓存，原文不修改。
- 修复 Guard 预算只能上调的问题，不能容纳必需模板时明确拒绝调用；Claude Code 按 UTF-8 字节限制输出并拒绝截断成功。
- 模型桥总期限覆盖预检及候选遍历，AGY 接纳排队和内部候选共用剩余期限，到期不迟发；Guard 不重复预检、不在结果未知后自动再次请求，关键阶段及时持久化。
- Windows AGY/Claude Code 使用 Job Object 在恢复执行前绑定子进程，清理确认后才返回；父先退而后代仍存活时拒绝部分输出。打包原生启动器源码，临时编译与校验失败时关闭调用，不回退到不受控启动。
- Grok 已提交请求后响应体断流标记为结果未知，模型桥和 Guard 不自动追加模型请求；明确未提交的连接拒绝及完整 HTTP 错误仍按原分类处理。
- AGY 默认 Flash 标签更新为当前 CLI 支持的 3.8 High/Medium，保留 Pro Low 候选及显式链路边界；不改 Antigravity LS 的模型键。
- 增加真实 Windows 假 CLI、持久缓存升级、序号与内容完整性、预算和输出边界回归。

## [1.25.4] - 2026-09-26

- 修复对话工具摘要、简略回复、角色读取及搜索上下文截断非BMP字符后生成孤立UTF-16代理码元，避免合法原文被格式化成严格JSON接口无法接收的请求。
- Stage Guard的输入预算、证据摘录和定位片段使用同一完整码点边界，保留既有长度预算、截断标记和原始数据，不以放宽模型上限或替换通道掩盖错误。
- 同步保护Codex上游摘要及Record相关字符切片；旧工具摘要仅在展示时从完整缓存字段重建，无法重建则明确显示码元转义，不重写原缓存。
- 增加emoji、扩展汉字、搜索前后边界、多个provider预算和未截断多语言内容的隔离回归。

## [1.25.3] - 2026-09-26

- 修复 Codex 原路径历史重写后旧缓存阻断 fetch：旧字节边界、锚点或长度已失效时，放弃增量复用并从当前源完整重建，不要求手动清理缓存。
- 仅对旧快照的明确失配执行回退，保留当前源、继承历史、取消、文件读取与发布前一致性校验；重建失败仍保留上一代缓存并明确报告失败。
- 增加原路径变长、等长锚点变化、缩短、正常追加、稳定命中、损坏当前源、冻结快照、取消和缺失源的隔离回归。

## [1.25.2] - 2026-09-19

- 修复 Record 指定工作区读写不一致：read/search/guide 优先该工作区的有效副本，避免旧副本因轮次较多覆盖刚编辑的内容；无本地副本或省略工作区时保留旧回退规则。
- 前缀与标题定位按不同对话身份判断歧义，而不是把同一对话的多个工作区副本计为多个候选。
- 手动 edit 拒绝写入有效 superseded 副本并提示目标工作区，不自动跨工作区写入；保留循环、失效归属的原有容错，不自动修改或删除历史数据。
- 增加隔离回归覆盖显式工作区、旧副本、归属链与循环、编辑后读取/搜索、索引模式以及拒绝编辑的无副作用边界。

## [1.25.1] - 2026-09-15

- Devin 身份识别使用只读 SQLite 字段投影，目标读取只加载目标主链及已核验的 Desktop 副本，避免无关会话正文消耗每次定位预算；保留强消息 ID 匹配、断链检查和读取期间身份变化拒绝。
- 上下文定位直接使用规范化正文，不生成图片附件或重复构建完整缓存；不提高默认预算，显式预算与取消仍有效。
- 区分一个命中但扫描未完成与多个真实命中的歧义，返回候选局部错误；主对话列表将父子命中折叠到相同展示身份，显式 all/children 范围不强行合并。
- Guard evidence 兼容字符串数组并在入口按行规范化，后台参数仍为字符串；子代理别名去重并补齐 UUID 形式，规范化版本升至 2。
...
- 将完整历史清单纳入来源指纹和后台 fetch 快照；同任务更换来源文件或继承关系时重建缓存，只有同一来源链的追加允许增量复用。
- 冷读、流式缓存、来源证据与后台阈值使用同一历史来源，保留旧格式读取并拒绝缺失、歧义、循环及边界不一致的继承链。
- 增加合成来源链和桥接缓存回归测试，覆盖回退、重复读取、追加、固定快照、跨逻辑任务继承以及异常来源。
...
- 从已发布 fetch 缓存直接执行范围导出时，Markdown 头部现在按实际选中轮次统计，不再把节省内存用的空占位数组误报为 0 轮。
