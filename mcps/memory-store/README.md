...
- **Codex 标题全量轻量定位与 Record 写入门禁** (v1.15.11+)：Codex 标题/ID/工作区/来源查询使用 `session_index.jsonl` 等轻量元数据，不再被最近 300 条正文候选限制；`record_manage(update/batch_update)` 与自动后台 Record 写入前会统一拒绝 `0 Phase`、覆盖轮次不足、异常缩水或 Phase 范围重叠的候选，候选保存到 `memory-store/temp`，旧正式 Record 保持不变。
...
在接收方对应宿主的 MCP 配置中添加；如果使用 Antigravity，通常是 `%USERPROFILE%\.gemini\antigravity\mcp_config.json`：

```json
{
  "memory-store": {
    "command": "node",
    "args": ["%USERPROFILE%\\path\\to\\toolkit\\mcps\\memory-store\\dist\\index.js"],
    "env": {
      "MEMORY_STORE_DATA_ROOT": "%USERPROFILE%\\.codex-toolkit\\memory-store"
    },
    "disabled": false
  }
}
