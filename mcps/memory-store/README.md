# MCP Memory Store v1.25.3

Record 指定 `workspace` 与对话时，read/search/guide 优先使用该工作区副本，并遵循有效的归属替代关系，不再被其它工作区的较高轮数覆盖。未指定工作区或本地没有副本时保留原有回退；同一对话的多份副本不会造成前缀或标题的假歧义。编辑已被有效替代的副本会明确拒绝并提示目标工作区，不自动跨工作区写入，也不迁移或删除历史副本。
...
支持 `session_meta.history_base` 与双 UUID rollout 文件：从当前线程表指定的叶文件追溯物理来源，严格按字节及全局记录截止位置读取继承前缀。回退或分叉后排除的旧尾部不会重新进入当前历史；缺失、歧义、循环和边界不一致会使读取失败，而不会把局部记录称为完整历史。

来源指纹包含完整继承清单。旧缓存首次访问时会在保留既有代次的基础上重建；同一任务更换叶文件或继承关系时也重新构建，只有同一来源链的正常追加允许增量读取。后台 fetch 的体积门槛使用全部有效前缀的总字节数，不只看最新文件。完整来源证据遇到未完成的末行会明确拒绝发布。

`npm run test:codex-history` 运行独立合成来源链及缓存桥接回归，不依赖真实用户历史。
