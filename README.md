...
| 组件 | 版本 | 主要用途 |
| --- | ---: | --- |
| `memory-store` | 1.22.7 | 记忆、四宿主 Conversation/Recall、Record、Golden Extract、Stage Guard、调度恢复与跨宿主路由 |
...
- Memory Store 1.22.7 可在 Windsurf/Antigravity 离线时列出本地 PB，通过 auto/manual/full Recall 恢复 context-only 上下文，自动迁移旧版或重复轮号的 fetch 缓存，让范围读取保留整段本地 PB 的 AI 回复与工具调用统计，并把自动通道 wake/alert 模板压缩为不冒充用户原话的结构化短事件。
...
| --- | ---: | --- |
| `memory-store` | 1.22.7 | Memory, four-host Conversation/Recall, Record, Golden Extract, Stage Guard, scheduling recovery, and host routing |
...
- Memory Store 1.22.7 can list local Windsurf/Antigravity PB files offline, restore context-only history through auto/manual/full Recall, migrate old or duplicate-round fetch caches, preserve full local PB response/tool totals during range reads, and condense automatic channel wake/alert templates into structured events instead of user-authored text.
