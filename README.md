...
| `sandbox` | 1.16.2 | 隔离执行、全局内存调度、按需 artifact、自适应输出、持久会话、Codex 任务与多模型 Council |
...
- Memory Store 1.22.4 可在 Windsurf/Antigravity 离线时列出本地 PB，通过 auto/manual/full Recall 恢复 context-only 上下文，并自动迁移旧 fetch 缓存，避免历史结构化 AI 字段导致 search 类型错误。
...
| `sandbox` | 1.16.2 | Isolated execution, global memory admission, on-demand artifacts, adaptive output, sessions, Codex tasks, and multi-model Council |
...
- Memory Store 1.22.4 can list local Windsurf/Antigravity PB files offline, restore context-only history through auto/manual/full Recall, and automatically migrate old fetch caches so historical structured AI fields cannot crash search.
