...
| 组件 | 版本 | 主要用途 |
| --- | ---: | --- |
| `memory-store` | 1.25.1 | 记忆、五宿主 Conversation/Recall、Record、Golden Extract、Stage Guard、调度恢复与跨宿主路由 |
...
- `conversation_read_original` 可列出、定位、搜索、分轮读取和导出 Codex、Antigravity、Claude Code、Windsurf 与 DeepSeek Harness 对话。
...
- Memory Store 1.25.1 在 WSF 数据链内自动兼容 Devin Local SQLite 与旧 Cascade LS/PB，支持文字 ID/已验证 UUID、原生子代理、只读缓存及五源限定上下文定位；定位按轻量身份和目标正文读取，区分单一已知命中与完整范围唯一。Devin SQLite 功能要求 Node.js 22.16+，推荐 24。
...
| --- | ---: | --- |
| `memory-store` | 1.24.0 | Memory, five-host Conversation/Recall, Record, Golden Extract, Stage Guard, scheduling recovery, and host routing |
| `sandbox` | 1.17.3 | Isolated execution, adaptive memory admission, streaming search isolation, Windows process-tree hard limits, on-demand artifacts, sessions, Codex tasks, and multi-model Council |
...
- Conversation tools can locate, search, read by round, and export conversations from Codex, Antigravity, Claude Code, Windsurf, and DeepSeek Harness.
...
- Memory Store 1.24.0 follows Codex referenced history and same-thread rollout replacement. Bounded inherited prefixes, cache fingerprints, background snapshots, and workload estimates share the complete source manifest.
