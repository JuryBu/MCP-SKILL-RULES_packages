...
| 组件 | 版本 | 主要用途 |
| --- | ---: | --- |
| `memory-store` | 1.22.3 | 记忆、四宿主 Conversation/Recall、Record、Golden Extract、Stage Guard、调度恢复与跨宿主路由 |
| `sandbox` | 1.16.0 | 隔离执行、全局内存调度、自适应输出、持久会话、Codex 任务与多模型 Council |
...
- Memory Store 1.22.3 可在 Windsurf/Antigravity 离线时列出本地 PB，对四宿主保存压缩元数据，并通过 auto/manual/full Recall 从同一 fetch generation 恢复 context-only 上下文。
...
| --- | ---: | --- |
| `memory-store` | 1.22.3 | Memory, four-host Conversation/Recall, Record, Golden Extract, Stage Guard, scheduling recovery, and host routing |
| `sandbox` | 1.16.0 | Isolated execution, global memory admission, adaptive output, sessions, Codex tasks, and multi-model Council |
...
- Memory Store 1.22.3 can list local Windsurf/Antigravity PB files offline, retain four-host compaction metadata, and restore context-only history through auto/manual/full Recall from one committed fetch generation.
