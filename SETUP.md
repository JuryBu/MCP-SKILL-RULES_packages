...
`Update-CodexMcpBroker.ps1` 与同目录的 `Start-CodexMcpBroker.ps1`、`Stop-CodexMcpBroker.ps1` 必须一起分发。更新器会把双布局兼容的启停脚本安装到扁平 broker 目录，因此目标机即使旧安装缺少 Stop 脚本也能受控升级；`-WhatIf` 只做预检，不创建备份或修改目标目录。
