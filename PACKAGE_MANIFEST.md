...
- Common MCP versions are memory-store 1.22.5, web-fetcher 7.0.0, sandbox 1.16.4, broker 0.1.0, and Windsurf-only mcp-subagent 1.1.0.
- Memory Store 1.22.5 retains four-host persistent fetch generations, offline PB listing and context-only Recall, automatically migrates old fetch caches, and preserves full local PB response/tool totals for range reads without packaging conversations or Plan/Task files.
