...
| Area | Contents |
| --- | --- |
| MCP | memory-store 1.25.1, web-fetcher 7.0.0, sandbox 1.17.3, broker 0.1.0 |
...
- Memory Store 1.25.1 routes Devin Local SQLite and legacy Cascade LS/PB through the existing WSF chain, with verified UUID/text aliases, native child discovery, cache-only lookup and scoped context matching across five sources. Context lookup uses lightweight identity discovery and selected-source reads, distinguishes a single observed match from complete-scope uniqueness, and normalizes parent/child identity counts to the requested list view. Devin SQLite requires Node.js 22.16+; Node.js 24 is recommended.
...
## 2026-09-12 Sandbox update

- Sandbox 1.17.3 adds incident-shaped low-memory admission replay, full-pressure freshness checks on Windows, per-lease observation credit, and explicit blocking decisions. Only existing unobserved reservations receive the configurable 25% physical weighting when complete commit accounting preserves 4096MB; new requests, aged protection, and commit accounting remain undiscounted.
- Launch maintenance becomes incremental and asynchronous, does not rewrite unchanged terminal tasks, keeps recovery admission blocked until existing leases are accounted for, and preserves task identity and deletion boundaries during concurrent queries. Focused tests include real Windows child execution and 1500 synthetic terminal records, without stressing the production host's memory.
...
- Common MCP versions are memory-store 1.25.1, web-fetcher 7.0.0, sandbox 1.17.3, broker 0.1.0, and Windsurf-only mcp-subagent 1.1.0.
...
- Memory Store 1.23.10 fixes WSF/Antigravity Record recovery when old Record metadata looks current but the body uses an incompatible legacy round universe; verified fetch cache ordering now drives stale detection, terminal scheduler retries are unblocked, and incompatible legacy bodies rebuild from current cache under the existing quality gates.
