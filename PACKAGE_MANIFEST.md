...
| Area | Contents |
| --- | --- |
| MCP | memory-store 1.22.4, web-fetcher 7.0.0, sandbox 1.16.1, broker 0.1.0 |
...
- Common MCP versions are memory-store 1.22.4, web-fetcher 7.0.0, sandbox 1.16.1, broker 0.1.0, and Windsurf-only mcp-subagent 1.1.0.
...
- sandbox 1.16.1 retains the 1.16.0 backend-wide memory admission and adaptive 100K-token/2000-line/1MiB delivery, while keeping six-hour output artifacts only for overflow, explicit file/manifest delivery, and interrupted recovery. Receiver status and expiry cleanup now report retained output-artifact counts.
